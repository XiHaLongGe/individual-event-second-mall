(function ($) {
	$.Tupload = {
		...
		createHtml:function(defaults){
			...
			html += 	'<ul>';
				...
				for(var i=0;i<fileNum;i++){
					...
					html += 	'<img id="img_src'+i+'" class="upload_image" src="/static/background/product-inf/images/imgadd.png"/>';
					html += '</div>';
		...
		imgLoad:function(i,file){
			...
			$(r).load(function(){
				while(true){
					if($("#img_src"+i).attr("src") != "/static/background/product-inf/images/imgadd.png"){
						i++;
					}else{
						break;
					}
		...
		formUpload:function (i,fileObj,FileController){
			...
			form.append("productInfId", $("#productInfIdINPUT").val());                        // 可以增加表单数据
			form.append("order", i);
			/*var multipartFile;
			if(Object.prototype.toString.call(Blob) == "[object Function]"){
				multipartFile = new File(fileObj, "multipartFile");
			}*/
		...
		onSuccess:function(data){
			return $.Tupload.options.onSuccess(data);
		},
		onDelete:function(i){
			...
			arrFile[i] = '';
			$("#img_src"+i).attr("src","/static/background/product-inf/images/imgadd.png");
			$("#uploading-tip"+i).hide();
			$.Tupload.setPhotoImg();
		}
	}
...
$("#fileImage").live('change',function(){
	...
	if(num < $.Tupload.fileNum+1){
		...
		for(var i=0;i<num;i++){
			for(var j=0;j<num;j++){
				if($("#img_src"+j).attr("src") == "/static/background/product-inf/images/imgadd.png"){
